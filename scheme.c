...
// This function blatantly stolen from the KOS Lua example.
...
#define ts_readline(L,b,p) \
        ((void)L, fputs(p, stdout), fflush(stdout),  /* show prompt */ \
        kbd_readline(b, LUA_MAXINPUT) != 0)  /* get line */
#define ts_saveline(L,idx)	{ (void)L; (void)idx; }
#define ts_freeline(L,b)	{ (void)L; (void)b; }

int kbd_sscanf ( const char * s, const char * format, ...){
...
void load_and_execute_scheme_file(scheme *sc, const char *file_name) {
    FILE *fin = open_file(file_name);
    if (fin == NULL) {
    ...
    if (sc->retcode != 0) {
        fprintf(stderr, "Errors encountered processing %s\n", file_name);
    }
}
...
int main(int argc, char **argv) {
   ...
   // load_and_execute_scheme_file(&sc, "rd/init.scm"); -- I looked at this but I have no idea
   // how needed it is.. and I don't know scheme so .... :D the goal is to get some scheme to print to the
   // console and that is what hello world does.
    dbgio_dev_select("fb");
    load_and_execute_scheme_file(&sc, "rd/init.scm");
    load_and_execute_scheme_file(&sc, "rd/examples/guess-game.scm");
  // create a hello_world.scm in the romdisk folder contents -
  // (display "Hello, World!")
  // (newline)
